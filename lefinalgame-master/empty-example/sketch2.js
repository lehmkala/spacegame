...
function draw() {
  ...
  if(aviao2.x<750&&ndead==true){aviao2.x+=inimigovelocidadex;aviao2.y+=inimigovelocidadey;}else{aviao2.x=random(-200);aviao2.y=random(-250);ndead=true;}//se inimigo nao tiver sido atingido ou ainda estiver dentro da tela movimente senão de respawn em origem randomica
    ...
    ndead=false;lu-=1000}//se tiver colisao da bala do aviao com inimigo set status de vivo para falso, dessa forma ele liga o respawn
       nu=dist(balax,balay,aviao1.x+70,aviao1.y+60);
        na=dist(aviao1.x,aviao1.y,aviao2.x,aviao2.y);
        if(nu<40||na<40){
    aviao1.x+=1000;scorevida--;aviao2.y+=1000;}
   ...
   // if(value==1){
    ...
    if(counter2>9){
     ...
     text(menucounter,334,643);//speed
     text(scorevida,528,643);//power
...
function keyPressed() {//aperta enter para  menu
  if (keyCode === 13) {
    if(menucounter==1){menu=!menu;menucounter=2;}else if(menucounter==2){menucounter=dificuldade;}else if(menucounter==3){menucounter=2;dificuldade=3;inimigovelocidadex=4;inimigovelocidadey=4;}else if(menucounter==4){menucounter=2;dificuldade=4;inimigovelocidadex=4;inimigovelocidadey=5;}else if(menucounter==5){menucounter=2;}
      if(scorevida<1){scorevida=3;menucounter=2;score=auxiliar;console.log(menucounter);}
  }
}
